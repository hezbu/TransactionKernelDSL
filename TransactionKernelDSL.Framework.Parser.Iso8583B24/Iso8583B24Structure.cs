...
{
    public sealed class Iso8583B24Structure : AbstractTransactionParserStructure
    {
        #region Member Field
        ...
        #region Constructor
        public Iso8583B24Structure(Nullable<AbstractTransactionParserStructureType> type,
                               string rootSection = "Iso8583B24",
            ...
            : base(rootSection, type)
        {
            ...
            {            
                Iso8583B24Field newField = new Iso8583B24Field()
                {
                    Id = f.Id,
                    Keyname = f.Keyname,
        ...
        public byte GetBitmapByByte(int intBmpIndex)
        {
            if (intBmpIndex > 16 || intBmpIndex < 0)
                throw new ArgumentException("Bitmap index must be within 0 and 15, but never " + intBmpIndex.ToString());
            ...
            {
                byteToReturn = AbstractTransactionFacade.GetHexaBytes(AbstractTransactionFacade.GetString(this[_SecondaryBitmapKeyname].Content, 2, (intBmpIndex - 8) * 2));
