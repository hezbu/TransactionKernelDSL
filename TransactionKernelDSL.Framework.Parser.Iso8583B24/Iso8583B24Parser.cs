...
{
    public sealed class Iso8583B24Parser : AbstractTransactionParser, ITransactionParserCommunicable, ITransactionParserAssembleable
    {
        private bool _IsBitmap128 = false;

        public Iso8583B24Parser(string rootSection = "Iso8583B24", bool isListenerParser = true) :
            base(rootSection, isListenerParser)
        {
            ...
            this._RequestStructure = new Iso8583B24Structure((_IsInputParser == true) ? AbstractTransactionParserStructureType.Request : AbstractTransactionParserStructureType.Response, _RootSection);
            this._ResponseStructure = new Iso8583B24Structure((_IsInputParser == true) ? AbstractTransactionParserStructureType.Response : AbstractTransactionParserStructureType.Request, _RootSection);
            this._RequestStream = new Iso8583B24Stream();
            this._ResponseStream = new Iso8583B24Stream();
        }
        ...
        public bool Receive(object handler)
        {
            ...
            int bytesToRead = Iso8583B24Stream.Iso8583B24MaxLength;
            byte[] btPartialReadsBuffer = new byte[Iso8583B24Stream.Iso8583B24MaxLength];
            byte[] btAccumulatedReadBuffer = new byte[Iso8583B24Stream.Iso8583B24MaxLength];
        ...
        public bool Assemble()
        {
            ...
            if (((Iso8583B24Structure)ResponseStructure).IsDataPrefix)
            {
                intLength += _ResponseStructure["DataPrefix"].CopyContentTo(ref partialData, intLength);
            }

            ///// OPTIONAL IMS_CICS TRANSACTION CODE
            if (((Iso8583B24Structure)ResponseStructure).IsImsCicsTransactionCode)
            {
            ...
            if ((((Iso8583B24Structure)ResponseStructure).GetBitmapByByte(0) & 0x80) == 0x80)
            ...
            for (intBmpIndex = 0; intBmpIndex < ((this._IsBitmap128 == true) ? 16 : 8); intBmpIndex++)
            {
                ...
                {
                    if (((int)((Iso8583B24Structure)ResponseStructure).GetBitmapByByte(intBmpIndex) & (mask << bitIndex)) == (mask << bitIndex))
                    {
            ...
            if (((Iso8583B24Structure)ResponseStructure).IsIso8583RequestMsgType())
            {
        ...
        public bool Disassemble()
        {
            ...
            try
            {
                ...
                int intLength = ((Iso8583B24Structure)_RequestStructure).FindISOStart(RequestStream.Get());
                if (intLength == 11)
                {
                ...
                if ((((Iso8583B24Structure)RequestStructure).GetBitmapByByte(0) & 0x80) == 0x80)
                {
                    this._IsBitmap128 = true;
                }
                else
                ...
                for (int intBmpIndex = 0; intBmpIndex < ((this._IsBitmap128 == true) ? 16 : 8); intBmpIndex++)
                {
                    for (int bitIndex = 7; bitIndex >= 0; bitIndex--)
                    {
                        if (((int)((Iso8583B24Structure)RequestStructure).GetBitmapByByte(intBmpIndex) & (mask << bitIndex)) == (mask << bitIndex))
                        {
                            intFieldFoundIndex = (intBmpIndex * 8) + (8 - bitIndex);
            ...
            catch (Exception ex)
            {
                ...
                (ResponseStructure["StartSignature"] as Iso8583B24Field).CopyContentFrom("ISO");
                (ResponseStructure["Base24Header"] as Iso8583B24Field).CopyContentFrom((RequestStructure["Base24Header"] as Iso8583B24Field).ToString().Substring(0, 4) + intFieldFoundIndex.ToString().PadLeft(3, '0') + (RequestStructure["Base24Header"] as Iso8583B24Field).ToString().Substring(7));
                (ResponseStructure["MessageType"] as Iso8583B24Field).CopyContentFrom("9"+(RequestStructure["MessageType"] as Iso8583B24Field).ToString().Substring(1));
                (ResponseStructure["PrimaryBitmap"] as Iso8583B24Field).CopyContentFrom("0000000000000000");
